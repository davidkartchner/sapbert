...
class Model_Wrapper(object):
    ...
    def load_bert(self, path, max_length, use_cuda, lowercase=True):
        print(path)
        self.tokenizer = AutoTokenizer.from_pretrained(path, do_lower_case=lowercase)
    ...
    def get_score_matrix(
        self, query_embeds, dict_embeds, cosine=False, normalise=False
    ):
        """
        Return score matrix
    ...
    def retrieve_candidate_cuda(
        self, score_matrix, topk, batch_size=128, show_progress=False
    ):
        """
        Return sorted topk idxes (descending order)
        ...
        for i in tqdm(
            ...
            matrix_sorted = torch.argsort(score_matrix_tmp, dim=1, descending=True)[
                :, :topk
            ].cpu()
            if res is None:
                res = matrix_sorted
